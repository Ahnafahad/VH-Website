// Script to add DU FBS students to the system
// Run with: node scripts/add-du-fbs-students.js
...
const students = [
  ...
  { studentId: '5074136', name: 'Ramisa Rahman', email: '20241110064040@vh.com' }, // Suspicious email
  { studentId: '6197380', name: 'Sayaf Bin Abdullah', email: 'b.a.sayaf17@gmail.com' },
  { studentId: '7842103', name: 'Shafqat Nur', email: '7842103@vh.com' } // Generated placeholder email
];

async function addFBSStudents() {
  try {
    // Use production URL or local based on environment
    const baseUrl = process.env.VERCEL_URL || 'https://vh-beyondthehorizons.vercel.app';
    // Or use local: 'http://localhost:3000'

    console.log(`\n🚀 Starting to process ${students.length} DU FBS students...\n`);
    console.log(`📍 Using URL: ${baseUrl}\n`);

    const response = await fetch(`${baseUrl}/api/add-fbs-students`, {
      method: 'POST',
      headers: {
    ...
    // List students with suspicious emails
    ...
    console.error('   2. You are logged in as an admin in your browser');
    console.error('   3. The database connection is working');
  }
}
