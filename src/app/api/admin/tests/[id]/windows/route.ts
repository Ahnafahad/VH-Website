 ...
 *         closesAt: number, durationMinutes?: number, classSessionId?: number|null }
 * Online windows require durationMinutes; offline windows are the entry slot.
 * classSessionId is the explicit test<->class link (rec #5) — which class
 * session this sitting belongs to, used by attendance auto-pull.
 */
...
import { tests, testWindows, classSessions } from '@/lib/db/schema';
...
export async function POST(
  ...
  return safeApiHandler(async () => {
    ...
    const { mode, opensAt, closesAt, durationMinutes, classSessionId } = parsed.data;
