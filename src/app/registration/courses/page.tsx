...
const MOCK_LABELS: Record<MockProgram, { title: string; short: string; detail: string }> = {
  'du-iba':  {
    title:  'Dhaka University — Institute of Business Administration (DU IBA)',
    short:  'DU IBA',
    detail: '10 comprehensive mocks · First mock free',
  },
  'bup-iba': {
    title:  'Bangladesh University of Professionals — Institute of Business Administration (BUP IBA)',
    short:  'BUP IBA',
    detail: '8 comprehensive mocks · First mock free',
  },
  'du-fbs':  {
    title:  'Dhaka University — Faculty of Business Studies (DU FBS)',
    short:  'DU FBS',
    detail: '8 comprehensive mocks · First mock free',
  },
  'bup-fbs': {
    title:  'Bangladesh University of Professionals — Faculty of Business Studies (BUP FBS)',
    short:  'BUP FBS',
    detail: '8 comprehensive mocks · First mock free',
  },
};

const FULL_LABELS: Record<FullCourse, { title: string; short: string; detail: string }> = {
  'iba-combined': {
    title:  'Dhaka University & Bangladesh University of Professionals — Institute of Business Administration (DU IBA & BUP IBA)',
    short:  'DU IBA & BUP IBA',
...
export default function CoursesRegistrationPage() {
  ...
  const toggleMock = (p: MockProgram) => {
    setMockIntent(null);
    setMocks(prev => prev.includes(p) ? prev.filter(x => x !== p) : [...prev, p]);
  };
      ...
      <div className="relative min-h-screen bg-[#FAF5EF] text-[#1A0507] overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-6 sm:px-10 pt-20 pb-24">
          ...
          <div className="border-t border-b border-[#D4B094]/30 py-6 my-10 space-y-3">
            {[
              ['Program', mode === 'mocks' ? mocks.map(m => MOCK_LABELS[m].short).join(' · ') : fullCourses.map(c => FULL_LABELS[c].short).join(' · ')],
              ...
              <div className="space-y-8">
                  ...
                  {(['hsc', 'alevels'] as const).map(t => (
                    <ToggleCard
                      ...
                      onClick={() => {
                        setEduType(t);
                        if (t === 'hsc') {
                          setMocks(prev => prev.filter(m => HSC_VISIBLE_MOCKS.includes(m)));
                          setFullCourses(prev => prev.filter(c => HSC_VISIBLE_FULL.includes(c)));
                        }
                      }}
                ...
                {eduType === 'hsc' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-4 border-t border-[#D4B094]/25">
                    <CourseInput label="HSC passing year" id="hsc" type="text" value={hscYear} onChange={setHscYear} placeholder={String(year)} />
                    <CourseInput label="SSC passing year" id="ssc" type="text" value={sscYear} onChange={setSscYear} placeholder={String(year - 2)} />
                  </div>
                )}
                {eduType === 'alevels' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-4 border-t border-[#D4B094]/25">
                    <CourseInput label="A Level passing year" id="al" type="text" value={aLevelYear} onChange={setALevelYear} placeholder={String(year)} />
                    <CourseInput label="O Level passing year" id="ol" type="text" value={oLevelYear} onChange={setOLevelYear} placeholder={String(year - 2)} />
                  </div>
                )}
              ...
              <div className="space-y-8">
                ...
                {mode === 'mocks' && (
                  <div className="space-y-6">
                    <div className="border-t border-[#D4B094]/25 pt-6">
                      ...
                      {eduType === 'hsc' && (
                        <div className="mb-5 rounded-xl border border-[#A86E58]/30 bg-[#A86E58]/[0.06] px-4 py-3">
                          <p className="font-sans text-[10px] tracking-[0.3em] uppercase text-[#A86E58] mb-1.5">
                            A note for HSC students
                          </p>
                          <p className="font-sans text-sm text-[#5A0B0F]/80 leading-relaxed">
                            These programs are designed primarily for English-medium (A-Level) students. You&rsquo;re welcome to register — prepare accordingly.
                          </p>
                        </div>
                      )}
                      ...
                      <div className="space-y-3">
                        {(eduType === 'hsc'
                          ? HSC_VISIBLE_MOCKS
                          : (['du-iba', 'bup-iba', 'du-fbs', 'bup-fbs'] as MockProgram[])
                        ).map(p => (
                          <MockCard
                            key={p}
                            program={p}
                      ...
                      {eduType === 'hsc' && mocks.length > 0 && (
                        <p className="mt-4 font-sans text-xs text-[#A86E58]/90 leading-relaxed">
                          Heads up — the selected program is English-medium oriented. Weigh the fit before confirming.
                        </p>
                      )}
                    </div>
                ...
                {mode === 'full' && (
                  <div className="space-y-4 border-t border-[#D4B094]/25 pt-6">
                    ...
                    {(eduType === 'hsc'
                      ? HSC_VISIBLE_FULL
                      : (['iba-combined', 'du-fbs-full', 'bup-fbs-full'] as FullCourse[])
                    ).map(c => (
                      <ToggleCard
                        key={c}
                        selected={fullCourses.includes(c)}
                        onClick={() => toggleFull(c)}
                        kicker={FULL_LABELS[c].short}
                        title={FULL_LABELS[c].title}
...
function ToggleCard({
  selected, onClick, kicker, title, detail,
}: { selected: boolean; onClick: () => void; kicker: string; title?: string; detail: string }) {
  return (
    <button
      onClick={onClick}
      className={[
    ...
    >
      ...
      {title && (
        <div className={`font-sans text-[11px] leading-snug mb-2 pr-8 ${selected ? 'text-[#D4B094]/80' : 'text-[#A86E58]/80'}`}>
          {title}
        </div>
      )}
...
function MockCard({
  program, selected, disabled, price, onToggle,
}: { program: MockProgram; selected: boolean; disabled: boolean; price: number; onToggle: () => void }) {
  const { title, short, detail } = MOCK_LABELS[program];
  return (
    <button
      onClick={onToggle}
      disabled={disabled}
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className={`font-sans font-medium text-base mb-1 ${selected ? 'text-[#FAF5EF]' : 'text-[#1A0507]'}`}>{short}</div>
          <div className={`font-sans text-[11px] leading-snug mb-2 ${selected ? 'text-[#D4B094]/85' : 'text-[#A86E58]/85'}`}>{title}</div>
          <div className={`font-sans text-xs ${selected ? 'text-[#FAF5EF]/60' : 'text-[#1A0507]/50'}`}>{detail}</div>
        </div>
